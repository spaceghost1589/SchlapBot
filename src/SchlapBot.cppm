...
using sc2::Agent, sc2::ClientError, sc2::GameInfo, sc2::MapData, sc2::Point3D, sc2::search::CalculateExpansionLocations,
  sc2::Unit, sc2::UpgradeID, sc2::UpgradeIDToName, std::cerr, std::cout, std::string, std::vector;
...
export struct SchlapBot : Agent {
    ...
    void OnBuildingConstructionComplete(const Unit* building_) override {
        cout << UnitTypeToName(building_->unit_type) << "(" << building_->tag << ") constructed" << '\n';
    }

    void OnUnitCreated(const Unit* unit_) override {
        cout << UnitTypeToName(unit_->unit_type) << "(" << unit_->tag << ") was created" << '\n';
    }

    void OnUnitIdle(const Unit* unit_) override {
        cout << UnitTypeToName(unit_->unit_type) << "(" << unit_->tag << ") is idle" << '\n';
    }

    void OnUnitDestroyed(const Unit* unit_) override {
        cout << UnitTypeToName(unit_->unit_type) << "(" << unit_->tag << ") was destroyed" << '\n';
    }
    ...
    void OnError(const vector<ClientError>& client_errors, const vector<string>& protocol_errors) override {
