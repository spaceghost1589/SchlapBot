...
size_t CalculateQueries(float radius, float step_size, const Point2D& center,
    ...
    while (loc < 360.0F)
    {
        ...
        Point2D current_grid = Point2D(std::floor(point.x), std::floor(point.y));
...
std::vector<std::pair<Point3D, std::vector<Unit>>> Cluster(const Units& units, float distance_apart) {
    ...
    for (const auto* unit : units)
    {
        ...
        {
            clusters.push_back(std::pair(u.pos, std::vector{u}));
            continue;
        }

        // Otherwise append to that cluster and update it's center of mass.
        target_cluster->second.push_back(u);
        const float size = static_cast<float>(target_cluster->second.size());
...
std::vector<Point3D> CalculateExpansionLocations(const ObservationInterface* resources, QueryInterface* query,
    ...
    const vector<std::pair<Point3D, vector<Unit>>> clusters = Cluster(resources_, parameters.cluster_distance_);
    ...
    for (const auto& cluster : clusters)
    {
        if (parameters.debug_)
        {
            ...
            {
                parameters.debug_->DebugSphereOut(get<0>(cluster), r, Colors::Green);
            }
        }
        ...
        for (const auto r : parameters.radii_)
        {
            query_count += CalculateQueries(r, parameters.circle_step_size_, get<0>(cluster), queries);
        }
    ...
    {
        const auto& [pt3d, vUnit] = clusters[i];
        float distance = std::numeric_limits<float>::max();
        Point2D closest;
        ...
        for (size_t j = start_index, e = start_index + query_size[i]; j < e; ++j)
        {
            ...
            if (const float d = Point2D::Distance(p, pt3d);
                d < distance)
            {
                distance = d;
                closest = p;
        ...
        const Point3D expansion(closest.x, closest.y, vUnit.begin()->pos.z);
