...
static int WebSocketDataHandler(mg_connection* conn, int /*flags*/, const char* data, size_t len,
...
static void WebSocketCloseHandler(const mg_connection* conn, void* websocket_server) {
    ...
    {
        if (const mg_connection* target = const_cast<mg_connection*>(server->connections_[i]);
            target == conn)
        {
...
static void SendMessage(mg_connection* conn, std::queue<T>& message_queue) {
    ...
    const google::protobuf::Message* message = message_queue.front().second;
...
bool Server::Listen(const char* listeningPorts, const char* requestTimeoutMs, const char* websocketTimeoutMs,
    ...
    mg_set_websocket_handler(mg_context_, "/sc2api", WebSocketConnectHandler, nullptr,
...
void Server::QueueRequest(mg_connection* conn, SC2APIProtocol::Request*& request) {
    request_mutex_.lock();
    requests_.push(RequestData(conn, request));
    request_mutex_.unlock();
}

void Server::QueueResponse(mg_connection* conn, SC2APIProtocol::Response*& response) {
    response_mutex_.lock();
    responses_.push(ResponseData(conn, response));
    response_mutex_.unlock();
}

void Server::SendRequest(mg_connection* conn) {
    request_mutex_.lock();
    SendMessage(conn ? conn : const_cast<mg_connection*>(connections_.front()), requests_);
    request_mutex_.unlock();
}

void Server::SendResponse(mg_connection* conn) {
    response_mutex_.lock();
    SendMessage(conn ? conn : const_cast<mg_connection*>(connections_.front()), responses_);
    response_mutex_.unlock();
}
