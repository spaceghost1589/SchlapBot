...
bool StartCivetweb() {
    ...
    const char* options[] = {
        "request_timeout_ms", "5000", "websocket_timeout_ms", "1200000", "num_threads", "4", "tcp_nodelay", "1", nullptr
    };
...
bool GetClientData(const mg_connection* connection, Connection*& out) {
    ...
    out = static_cast<Connection*>(mg_get_user_data(ctx));
    return true;
}

static int DataHandler(const mg_connection* conn, int /*flags*/, const char* data, size_t data_len, void*) {
...
static void ConnectionClosedHandler(const mg_connection* conn, void*) {
    Connection* sc2_connection;
    if (!GetClientData(conn, sc2_connection)) {
        return;
    }
...
bool Connection::Connect(const std::string& address, int port, bool verbose) {
    ...
    connection_ = mg_connect_websocket_client(address.c_str(), port, 0, ebuff, 256, "/sc2api", nullptr,
                                              reinterpret_cast<mg_websocket_data_handler>(DataHandler),
                                              ConnectionClosedHandler, static_cast<void*>(this));

    if (!connection_) {
        std::cerr << "Failed to establish websocket connection: " << ebuff << '\n';
        return false;
    }
...
void Connection::Send(const SC2APIProtocol::Request* request) const {
    if (!request) {
        return;
    }
...
bool Connection::Receive(SC2APIProtocol::Response*& response, unsigned int timeout_ms) {
    ...
    if (condition_.wait_until(lock, now + std::chrono::milliseconds(timeout_ms), [&] {
...
void Connection::SetTimeoutCallback(const std::function<void()>& callback) {
    timeout_callback_ = callback;
}

void Connection::SetConnectionClosedCallback(const std::function<void()>& callback) {
