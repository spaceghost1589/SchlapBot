...
bool Convert(const SC2APIProtocol::VitalScoreDetails& details_proto,
...
bool Convert(const SC2APIProtocol::CategoryScoreDetails& details_proto,
...
bool Convert(const ScorePtr& score_ptr, ScoreDetails& score_details) {
	...
	score_details.idle_production_time = score_details_ptr->
	...
	score_details.total_value_structures = score_details_ptr->
		total_value_structures();
	score_details.killed_value_units = score_details_ptr->killed_value_units();
	score_details.killed_value_structures = score_details_ptr->
	...
	score_details.collection_rate_minerals = score_details_ptr->
		collection_rate_minerals();
	score_details.collection_rate_vespene = score_details_ptr->
...
bool Convert(const ObservationRawPtr& observation_raw, UnitPool& unit_pool,
             uint32_t game_loop,
             uint32_t prev_game_loop) {
	for (int i = 0; i < observation_raw->units_size(); ++i)
	{
		const SC2APIProtocol::Unit& observation_unit = observation_raw->
			units(i);
		...
		if (bp >= 1.0f && unit->build_progress > 0.0f && unit->build_progress <
		    1.0f)
		{
		...
		unit->orders.clear();
		for (int order_index = 0; order_index < observation_unit.orders_size();
		     ++order_index)
		{
			const SC2APIProtocol::UnitOrder& order_proto = observation_unit.
				orders(order_index);
		...
		unit->buffs.clear();
		for (int buff_index = 0; buff_index < observation_unit.buff_ids_size();
		     ++buff_index)
		{
...
bool Convert(const SC2APIProtocol::DisplayType& type_proto,
...
bool Convert(const SC2APIProtocol::Alliance& alliance_proto,
...
bool Convert(const SC2APIProtocol::CloakState& cloak_proto,
...
//	if (!Convert(observation_render->minimap(), render.minimap))
	if (!render.minimap.UpdateImageDataLocal(observation_render->minimap(), true))
	{
		return false;
	}
...
void ConvertRawActions(const ResponseObservationPtr& response_observation_ptr,
                       RawActions& actions) {
	for (int i = 0; i < response_observation_ptr->actions_size(); ++i)
	{
		const SC2APIProtocol::Action& proto_action = response_observation_ptr->
			actions(i);
		...
		else if (action_raw_command.has_target_world_space_pos())
		{
			...
			action.target_point.x = action_raw_command.target_world_space_pos().
			                                           x();
			action.target_point.y = action_raw_command.target_world_space_pos().
			                                           y();
		}
...
void ConvertFeatureLayerActions(
	...
	for (int i = 0; i < response_observation_ptr->actions_size(); ++i)
	{
		...
		const SC2APIProtocol::ActionSpatial& action_FL = proto_action.
...
void ConvertRenderedActions(
	...
	for (int i = 0; i < response_observation_ptr->actions_size(); ++i)
	{
		...
		const SC2APIProtocol::ActionSpatial& action_render = proto_action.
			action_render();
		ConvertSpatialAction(action_render, actions);
	}
}

void Convert(const SC2APIProtocol::SpatialCameraSetup& setup_proto,
             SpatialSetup& setup) {
	setup.camera_width = setup_proto.width();
	const SC2APIProtocol::Size2DI& resolution = setup_proto.resolution();
	const SC2APIProtocol::Size2DI& minimap_resolution = setup_proto.
		minimap_resolution();
...
bool Convert(const SC2APIProtocol::InterfaceOptions& options_proto,
...
//! @
bool Convert(const ResponseGameInfoPtr& response_game_info_ptr,
             GameInfo& game_info) {
	if (!response_game_info_ptr->has_start_raw())
	{
		return false;
	}
	const SC2APIProtocol::StartRaw& start_raw = response_game_info_ptr->
		start_raw();
	...
	if (start_raw.has_playable_area())
	{
		...
		{
			game_info.playable_min.x = static_cast<float>(start_raw.
			                                              playable_area().p0().
			                                              x());
			game_info.playable_min.y = static_cast<float>(start_raw.
			                                              playable_area().p0().
			                                              y());
		}
		...
		{
			game_info.playable_max.x = static_cast<float>(start_raw.
			                                              playable_area().p1().
			                                              x());
			game_info.playable_max.y = static_cast<float>(start_raw.
			                                              playable_area().p1().
			                                              y());
		}
	}
	...
	for (int i = 0, e = start_raw.start_locations_size(); i < e; ++i)
	{
		...
		game_info.enemy_start_locations.push_back(Point2D(pt.x(), pt.y()));
	}
