...
export namespace sc2::CTRL_Face {

AppState app_state_ { AppState::normal };
...
unique_ptr<ObservationInterface> observation_face_ =
    make_unique<ObservationInterface> (
        observation_ptr, response_observation_ptr
    );
unique_ptr<QueryInterface> query_impl_ = make_unique<QueryInterface>( );
unique_ptr<DebugInterface> debug_impl_ =
    make_unique<DebugInterface> ( *observation_face_ );
ProcessInfo process_info_;

bool CTRL_Face::Connect ( const string& address, int port, int timeout_ms ) {
...
bool CTRL_Face::CreateGame (
    const string& map_name, const vector<PlayerSetup>& players, bool realtime
) {
...
bool CTRL_Face::IssueEvents ( const Tags& commands ) {
    ...
    void CTRL_Face::IssueUnitDestroyedEvents( ) const {
    ...
    void CTRL_Face::IssueUnitAddedEvents( ) const {
    ...
    void CTRL_Face::IssueBuildingCompletedEvents( ) const {
        for ( auto unit :
    ...
    void CTRL_Face::IssueIdleEvents ( const Tags& commands ) const {
    ...
    void CTRL_Face::IssueUpgradeEvents( ) const {
    ...
    void CTRL_Face::IssueAlertEvents( ) const {
    ...
    void CTRL_Face::IssueUnitDamagedEvents( ) const {
        for ( const auto& u : observation_face_->unit_pool_.GetDamagedUnits( ) )
        {
...
void CTRL_Face::OnGameStart ( ) {
    ...
    // Clear start locations here since CTRL_Face::OnGameStart is
...
// Diagnostic.
void CTRL_Face::DumpProtocolUsage ( ) {
...
bool CTRL_Face::RequestJoinGame (
    PlayerSetup              setup,
...
bool CTRL_Face::RequestLeaveGame ( ) {
    if ( !is_multiplayer_ )
    {
        return false;
    }
...
bool CTRL_Face::SaveReplay ( const string& path ) {
    GameRequestPtr request = ProtoFace::MakeRequest( );
...
GameResponsePtr CTRL_Face::WaitForResponse ( ) {
    ...
    Assert ( process_info_.process_id );
...
void CTRL_Face::SetProcessInfo ( const ProcessInfo& pi ) {
    process_info_ = pi;
}

const ProcessInfo& CTRL_Face::GetProcessInfo ( ) const {
    return process_info_;
}

// Game status.
SC2APIProtocol::Status CTRL_Face::GetLastStatus ( ) const {
    return ProtoFace::GetLastStatus( );
}

AppState CTRL_Face::GetAppState ( ) const {
    return app_state_;
}

bool CTRL_Face::IsInGame ( ) const {
    if ( app_state_ != AppState::normal )
    {
        return false;
    }
...
bool CTRL_Face::IsFinishedGame ( ) const {
    if ( app_state_ != AppState::normal )
    {
        return true;
    }
...
bool CTRL_Face::IsReadyForCreateGame ( ) const {
    if ( app_state_ != AppState::normal )
    {
        return false;
    }
...
bool CTRL_Face::HasResponsePending ( ) const {
    return ProtoFace::HasResponsePending( );
}

bool CTRL_Face::GetObservation ( ) {
    if ( app_state_ != AppState::normal )
    {
        return false;
    }
...
bool CTRL_Face::PollResponse ( ) {
    return ProtoFace::PollResponse( );
}

bool CTRL_Face::ConsumeResponse ( ) {
    const GameResponsePtr response = WaitForResponse( );
    return response.get( );
}

void CTRL_Face::ResolveMap (
