...
public:
    ...
    //! @param replay_info Replay information used to decide if the replay
    //! should be filtered.
    //! @param player_id
    //! @return If 'true', the replay will be rejected and not analyzed.
    virtual bool IgnoreReplay (
        const ReplayInfo& replay_info, uint32_t player_id
    );
...
public:
    ...
    virtual bool GatherReplayInfo (
        const std::string& path, bool download_data = false
    ) = 0;

    virtual bool LoadReplay (
