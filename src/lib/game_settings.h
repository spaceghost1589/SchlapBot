...
struct ProcessInfo {
    ...
    ProcessInfo(const std::string& path, uint64_t id, int port):
            process_path(path), process_id(id), port(port) {}
...
struct ProcessSettings {
    ...
    int    timeout_ms;
    int    port_start;
    bool   multi_threaded; // Run all OnSteps in parallel.
    bool   full_screen;
    vector<string>
...
struct FeatureLayerSettings {
    ...
    FeatureLayerSettings(float in_camera_width,
                         int   in_map_x,
                         int   in_map_y,
                         int   in_minimap_x,
                         int   in_minimap_y):
...
enum class AppState {
    ...
    timeout_zombie, // A timeout has occurred, but the game application could
                    // not be terminated.
