...
int LaunchProcess(ProcessSettings& process_settings, Client* client, int window_width, int window_height,
    ...
    if (!pi.process_id) {
        std::cerr << "Unable to start sc2 executable with path: " << process_settings.process_path << '\n';
    } else {
...
bool CoordinatorImp::ShouldRelaunch(ReplayObserver* r) {
    ...
    std::cout << "Replay is from a different version. Relaunching client into the correct version..." << '\n';
...
void Coordinator::LaunchStarcraft() {
    if (!DoesFileExist(imp_->process_settings_.process_path)) {
        std::cerr << "Executable path can't be found, try running the StarCraft II executable first." << '\n';
...
void Coordinator::Connect(int port) {
    if (!imp_->agents_.front()->Control()->Connect(imp_->process_settings_.net_address, port,
        ...
        std::cerr << "Failed to attach to starcraft." << '\n';
        exit(1);
    }
