...
bool ReplayControlImpl::LoadReplay(const std::string& replay_path, const InterfaceSettings& settings, uint32_t player_id,
                                  bool realtime) {
    // Send the request.
    GameRequestPtr request = control_interface_->Proto().MakeRequest();
...
bool ReplayControlImpl::WaitForReplay() {
    ...
    if (!response->has_start_replay()) {
        std::cerr << "WaitForReplay: received the wrong type of response: "
                  << std::to_string(int(response->response_case())) << '\n';
        assert(0);
        return false;
    }
    ...
    if (response_replay.has_error()) {
        ...
        std::cerr << "WaitForReplay: start replay contains an error: " << std::to_string(int(err)) << '\n';
