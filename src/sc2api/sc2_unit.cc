...
Unit* UnitPool::CreateUnit(const Tag tag) {
...
Unit* UnitPool::GetUnit(const Tag tag) const {
    auto found = tag_to_unit_.find(tag);
    return found == tag_to_unit_.end() ? nullptr : found->second;
}

Unit* UnitPool::GetExistingUnit(const Tag tag) const {
...
void UnitPool::MarkDead(const Tag tag) {
...
bool UnitPool::UnitExists(const Tag tag) {
